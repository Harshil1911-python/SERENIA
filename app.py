...
def submit_property():
    try:
        ...
        new_property = [
            prop_id, request.form['title'], request.form['property_type'],
        ...
        with open('properties.csv', 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow('id,title,property_type,category,full_address,city,bedrooms,bathrooms,area_sqft,sale_price,per_sqft_price,monthly_rent,seller_name,seller_phone,description,listed_date,photos,videos,amenities,documents'.split(','))
